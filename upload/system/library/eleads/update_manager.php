...
class EleadsUpdateManager {
	...
	private function refreshAfterUpdate() {
		...
		$this->loader->controller('extension/modification/refresh', array('redirect' => 'extension/module/eleads'));
	...
	private function syncLocalModification() {
		...
		$this->loader->model('extension/modification');

		$exists = $this->model_extension_modification->getModificationByCode($meta['code']);
		if (!empty($exists['modification_id'])) {
			$this->model_extension_modification->deleteModification((int)$exists['modification_id']);
		}
