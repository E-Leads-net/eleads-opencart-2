...
<div id="product-category" class="container eleads-filter-page"
     data-api-url="<?php echo $filter_api_url; ?>"
     data-project-id="<?php echo (int)$filter_project_id; ?>"
     data-language="<?php echo $filter_language; ?>"
     data-base-url="<?php echo $filter_base_url; ?>"
     data-initial-state='<?php echo htmlspecialchars($filter_state_json, ENT_QUOTES, 'UTF-8'); ?>'>
  <style>
    ...
    .eleads-filter-page .product-layout { margin-bottom: 20px; }
    .eleads-filter-page .product-thumb {
      height: 100%;
      display: flex;
      flex-direction: column;
    }
    .eleads-filter-page .product-thumb .image {
      min-height: 220px;
      display: flex;
      align-items: center;
    ...
    .eleads-filter-page .product-thumb .image img {
    ...
    .eleads-filter-page .product-thumb > div:last-child {
      display: flex;
      flex-direction: column;
      flex: 1;
    }
    .eleads-filter-page .product-thumb .caption {
      min-height: 170px;
      display: flex;
      flex-direction: column;
    }
    .eleads-filter-page .product-thumb .caption p {
      margin-bottom: 8px;
    }
    .eleads-filter-page .product-thumb .button-group {
      margin-top: auto;
    }
  </style>
  ...
  <div class="row">
    ...
    <div id="content" class="<?php echo $class; ?>">
      ...
      <div class="row eleads-toolbar">
        <div class="col-md-8 col-sm-6 col-xs-12">
          <input type="text" id="eleads-query" class="form-control" placeholder="<?php echo $heading_title; ?>" value="" />
        </div>
      ...
      <div id="eleads-status" class="eleads-status"><?php echo $text_loading; ?></div>

      <div class="row" id="eleads-products"></div>

      <div class="pagination-wrap">
        <button type="button" id="eleads-prev" class="btn btn-default"><?php echo $button_prev; ?></button>
        <span id="eleads-page-info" class="text-muted"></span>
        <button type="button" id="eleads-next" class="btn btn-default"><?php echo $button_next; ?></button>
      </div>
...
(function () {
  ...
  function buildSidebar(data) {
    ...
    html += '<div class="list-group-item header"><?php echo addslashes($text_category); ?></div>';
    html += '<button type="button" class="list-group-item' + (state.category ? '' : ' active') + '" data-role="cat" data-id=""><?php echo addslashes($text_all_categories); ?></button>';
  ...
  function buildProductCard(item) {
    ...
    var picture = item && item.picture ? String(item.picture) : 'image/cache/placeholder-200x200.png';
    ...
    return ''
      ...
      + '    <div class="image"><a href="' + escapeHtml(url) + '"><img src="' + escapeHtml(picture) + '" alt="' + escapeHtml(name) + '" title="' + escapeHtml(name) + '" class="img-responsive" /></a></div>'
      + '    <div>'
      + '      <div class="caption">'
      ...
      + '        <button type="button" class="js-eleads-cart" data-product-id="' + escapeHtml(productId) + '"><i class="fa fa-shopping-cart"></i> <span class="hidden-xs hidden-sm hidden-md"><?php echo addslashes($button_cart); ?></span></button>'
      + '        <button type="button" class="js-eleads-wishlist" data-product-id="' + escapeHtml(productId) + '" data-toggle="tooltip" title="<?php echo addslashes($button_wishlist); ?>"><i class="fa fa-heart"></i></button>'
      + '        <button type="button" class="js-eleads-compare" data-product-id="' + escapeHtml(productId) + '" data-toggle="tooltip" title="<?php echo addslashes($button_compare); ?>"><i class="fa fa-exchange"></i></button>'
      + '      </div>'
      + '    </div>'
      + '  </div>'
      + '</div>';
  ...
  function bindProductActionEvents() {
    productsEl.addEventListener('click', function (event) {
      ...
      if (cartBtn) {
        ...
        if (cartId && window.cart && typeof window.cart.add === 'function') {
          window.cart.add(cartId);
        }
        return;
      }
      ...
      if (wishBtn) {
        ...
        if (wishId && window.wishlist && typeof window.wishlist.add === 'function') {
          window.wishlist.add(wishId);
        }
        return;
      }
      ...
      if (cmpBtn) {
        ...
        if (cmpId && window.compare && typeof window.compare.add === 'function') {
          window.compare.add(cmpId);
        }
      }
    });
  }
  ...
  function loadData() {
    if (projectId <= 0) {
      statusEl.textContent = '<?php echo addslashes($text_no_project); ?>';
    ...
    statusEl.textContent = '<?php echo addslashes($text_loading); ?>';
    pushState();
...
<?php if (!empty($filter_custom_css)) { ?>
<style id="eleads-custom-css">
<?php echo $filter_custom_css; ?>
</style>
<?php } ?>
