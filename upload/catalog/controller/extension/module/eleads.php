...
class ControllerExtensionModuleEleads extends Controller {
	...
	private function authorizeApiRequest() {
	...
	private function jsonError($status_code, $error) {
		$status_map = array(
		...
		$this->response->addHeader(isset($status_map[$status_code]) ? $status_map[$status_code] : 'HTTP/1.1 500 Internal Server Error');
		$this->response->addHeader('Content-Type: application/json');
		$this->response->setOutput(json_encode(array('error' => $error)));
	}

	private function formatFeedJobStatus($result) {
		return array(
			'status' => isset($result['status']) ? $result['status'] : 'idle',
			...
			'finished_at' => isset($result['finished_at']) ? $result['finished_at'] : '',
	...
	public function seoPage() {
		...
		$data['order'] = 'ASC';
		$data['limit'] = (int)$this->config->get($this->config->get('config_theme') . '_product_limit');
		$data['sorts'] = array();
		$data['limits'] = array();
	...
	private function getProductDescriptionLength() {
		$theme = (string)$this->config->get('config_theme');
		$length = (int)$this->config->get('theme_' . $theme . '_product_description_length');
		if ($length <= 0) {
	...
	private function resolveStoreLanguageCode($lang) {
		...
		foreach ($languages as $language) {
			...
			} elseif (isset($language['language_id'])) {
				$code = (string)$language['language_id'];
			}
