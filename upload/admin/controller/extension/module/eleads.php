...
class ControllerExtensionModuleEleads extends Controller {
	...
	public function index() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && isset($this->request->post['module_eleads_api_key_submit'])) {
			...
			if ($api_key_submitted !== '' && $this->checkApiKeyStatus($api_key_submitted)) {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/eleads', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('extension/module/eleads', 'token=' . $this->session->data['token'], true);
		...
		if ($api_key_valid) {
			...
			$data['update_url'] = $this->url->link('extension/module/eleads/update', 'token=' . $this->session->data['token'], true);
		} else {
	...
	public function update() {
		...
		$this->response->redirect($this->url->link('extension/module/eleads', 'token=' . $this->session->data['token'], true));
	}

	private function refreshAfterUpdate() {
		// Rebuild OCMOD cache
		$this->load->controller('extension/modification/refresh', array('redirect' => 'extension/module/eleads'));
	...
	public function install() {
		...
		} elseif (is_file(DIR_APPLICATION . 'model/extension/event.php')) {
			$this->load->model('extension/event');
			$this->model_extension_event->addEvent('eleads_product_add', 'admin/model/catalog/product/addProduct/after', 'extension/module/eleads/eventProductAdd');
			$this->model_extension_event->addEvent('eleads_product_edit', 'admin/model/catalog/product/editProduct/after', 'extension/module/eleads/eventProductEdit');
			$this->model_extension_event->addEvent('eleads_product_delete', 'admin/model/catalog/product/deleteProduct/after', 'extension/module/eleads/eventProductDelete');
		}
	}
	...
	public function uninstall() {
		if (is_file(DIR_APPLICATION . 'model/setting/event.php')) {
			...
			$this->load->model('extension/event');
			$this->model_extension_event->deleteEvent('eleads_product_add');
			$this->model_extension_event->deleteEvent('eleads_product_edit');
			$this->model_extension_event->deleteEvent('eleads_product_delete');
		}
